...
def load_attrib():
    """Очередь поправок авторства — attrib.tsv.

    Постоянно жить здесь поправкам нельзя: пока они лежат рядом с данными,
    источников правды два, и разбор, начатый с таблицы, находит уже
    исправленное заново. Поэтому очередь применяется к самой книге
    (tools/fix-sheet.py) и опустевает; здесь она нужна лишь на то время,
    пока правка ещё не вписана.
...
def main():
    if ATTRIB:
        # Тихо применить и забыть — как раз тот случай, из-за которого
        # правки потом находятся по второму разу.
        print('  ! поправок авторства мимо таблицы: %d — '
              'впишите их: python3 tools/fix-sheet.py' % len(ATTRIB))
